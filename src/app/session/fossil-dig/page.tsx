    ...
    <div className="min-h-screen bg-[#faf8f4]">
      ...
      <div className="border-b border-[#e8e0d4] bg-white px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {["SETUP", "PRESENTING", "RECALLING", "ANALYZING", "REVEALING"].map(
            (phase, i) => (
              <div
                key={phase}
                  ...
                  className={`h-2 w-2 rounded-full transition-all ${
                    state.phase === phase
                      ...
                      : ["REDIG", "COMPLETE"].includes(state.phase) ||
                        ["SETUP", "PRESENTING", "RECALLING", "ANALYZING", "REVEALING"]
                          .indexOf(state.phase) > i
                      ? "bg-[#c4622d] opacity-40"
                      : "bg-[#e8e0d4]"
                  }`}
                />
                {i < 4 && (
                  <div className="h-px w-4 bg-[#e8e0d4]" />
                )}
              </div>
            )
          )}
