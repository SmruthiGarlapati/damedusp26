...
export default function SetupPanel({
  ...
  const canStart =
    ...
    (state.role === "scribe" || state.presenterNotes.trim().length > 0);
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-[#6b6b65] text-sm">
          One person teaches, the other listens and recalls. Both of you learn.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            {
              role: "presenter" as PlayerRole,
              emoji: "🦴",
              title: "Lead Paleontologist",
              desc: "You teach the topic. Upload your notes and present.",
            },
            {
              role: "scribe" as PlayerRole,
              emoji: "👂",
              title: "Field Scribe",
              desc: "You listen carefully, then recall everything from memory.",
            },
          ].map((r) => (
            <button
              key={r.role}
              ...
              className={`text-left rounded-2xl border-2 p-5 transition-all ${
                state.role === r.role
                  ? "border-[#c4622d] bg-[#fdf6f2]"
                  : "border-[#e8e0d4] bg-white hover:border-[#c4622d]/40"
              }`}
            >
              <div className="text-2xl mb-2">{r.emoji}</div>
              <div className="font-bold text-sm text-[#1a1a18] mb-1">
                {r.title}
              </div>
              <div className="text-xs text-[#9b9b95] leading-relaxed">
                {r.desc}
              </div>
            </button>
          ))}
      ...
      <div>
        ...
        <input
          ...
          className="w-full rounded-xl border border-[#e8e0d4] bg-white px-4 py-3 text-sm text-[#1a1a18] outline-none focus:border-[#c4622d] transition-colors placeholder:text-[#c4622d]/30"
        />
      </div>

      {/* Notes upload — presenter only */}
            ...
            {state.presenterNotes ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs text-[#9b9b95]">
                  {state.presenterNotes.length} characters
                </span>
                <button
                  onClick={() => setPresenterNotes("")}
                  className="text-xs text-[#c4622d] underline mt-1"
                >
                  Remove
                </button>
              </div>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-[#9b9b95]">or</p>
                <textarea
      ...
      <div>
        ...
        <div className="flex gap-3">
          {[5, 10, 15].map((mins) => (
            <button
              key={mins}
              onClick={() => setPresentationMinutes(mins)}
              className={`flex-1 rounded-xl border-2 py-3 text-sm font-bold transition-all ${
                state.presentationMinutes === mins
                  ? "border-[#c4622d] bg-[#fdf6f2] text-[#c4622d]"
                  : "border-[#e8e0d4] bg-white text-[#6b6b65] hover:border-[#c4622d]/40"
              }`}
            >
              {mins} min
            </button>
